...
export async function POST(req: Request) {
  try {
    ...
    const { tpPercent, slPercent, minWinRate, dataLimit, buyCommission = 0.1, sellCommission = 0.1, mode = 'manual', lookahead = 48 } = body;
    ...
    return new Promise<Response>((resolve) => {
      ...
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error('[AI DISCOVERY] Python Error:', errorData);
        ...
        try {
          ...
          console.error('[AI DISCOVERY] JSON Parse Error:', resultData);
